...
- **六种数据导入方式**：游戏目录、云鸣潮、抽卡链接、JSON、截图识别与批量手动录入
- **本地截图识别**：按需下载独立 OCR 组件，识别五星与抽数后再由用户校验，不上传截图
- **批量记录校验**：可修改角色或武器、抽数与顺序，也可添加、复制、拖拽或删除条目
- **多账号独立管理**：按 UID 分库存储、切换、统计、导出与清理，卡池数据互不混合
- **八类卡池独立统计**：分别计算累计抽数、五星区间、当前垫抽与保底进度
- **唤取分析页面**：提供历史中位数、理论中位数、个人概率校准、条件概率预测与五星分布
- **UP 表现复盘**：区分总体不歪率、歪/不歪次数与完整 UP 周期成本
- **五星获取成本**：点击列表、宫格或表格中的五星，可查看逐次获取、前置歪、总抽数与平均成本
- **多视角记录浏览**：列表、宫格、表格三种布局，支持卡池筛选、搜索与分页
- **规则校验的手动补录**：可按目标抽数插入五星，也可在指定时间补充普通抽数，按卡池规则生成三星、四星记录
- **模拟记录可编辑**：仅手动生成的数据允许编辑或删除，真实扫描/导入记录始终只读
- **同步新鲜度提醒**：记录最近完整同步时间，并按风险等级提示可能的数据缺口
- **历史起点管理**：可按 UID、按卡池确认完整起点，或按已知实际抽数为首个五星补足缺失的三星、四星模拟记录
- **安全导入与导出**：JSON 导入先预检，导入后展示变更摘要；分析和导出均可限定日期范围
- **OneDrive 云同步**：桌面端与 Android 端共享 `gacha-data.db` 数据快照，启动时拉取远端更新并使用 ETag 阻止并发覆盖
- **统一素材缓存**：角色、武器图标与角色立绘由独立资源包后台更新，OCR 复用同一套本地素材并保留远程兜底
...
| 文件导入 | 迁移或恢复 | 预检[本工具 JSON 示例](https://github.com/juliy819/wuwa-gacha-tool/blob/main/docs/examples/uid_999999999_mock_2026-07-31.json)、[WaveTools](https://github.com/JamXi233/WaveTools) 导出文件或兼容官方接口结构的 JSON |
| 截图识别 | 从其他抽卡工具迁移 | 本地识别截图中的五星与抽数，人工校验后批量写入 |
| 批量手动 | 没有可导出的原始数据 | 直接批量编辑五星、抽数、顺序与日期范围 |
...
![截图识别导入](docs/screenshots/ocr-import.png)

截图识别面向无法直接导出抽卡数据的第三方平台。OCR 在本机运行，识别结果不会直接写入数据库：

- 首次使用时按需下载独立 OCR 组件，可在页面内检查更新、修复或删除
- 自动识别不同截图排版，并根据识别到的角色或武器推测卡池大类
- 用户可切换目标卡池；记录类型与卡池不匹配时会明确标记并阻止导入
- 五星名称、抽数和顺序均可修改，支持添加、复制、拖拽排序和删除
- 截图不含日期时，可设置首尾日期，由工具按当前顺序分布记录时间
- 导入前会检查抽数、卡池规则、异常五星序列及与已有记录重叠的日期范围

识别记录与批量手动记录都会作为模拟记录写入。OCR 适合辅助迁移，不应把未人工核对的结果视为官方完整历史。
...
- 支持最新/最早排序、分页与跨页面精确定位；从首页或分析页跳转时会高亮目标记录
- 列表与宫格视图会直接显示当前垫抽，便于边浏览边判断卡池进度
...
- 从当前垫抽出发的条件概率预测，可在规则模型与基于个人历史收缩校准的模型间切换
...
个人校准以游戏规则模型为基线，再根据该 UID、该卡池的有效历史区间进行保守修正；样本越少，结果越接近规则模型。它用于观察个人历史差异，不会改变保底规则，也不代表未来结果承诺。
...
- 可限定开始、结束日期，仅导出指定时间范围内的记录
- 可按卡池确认或撤销历史起点，用于区分完整区间与仅有下界的早期数据
- 后台检查角色、武器资源包更新，安装新包后清理旧版图标缓存
- 单独删除某位玩家或清空全部记录；确认弹窗会明确展示影响范围
- 所有删除操作执行前先创建完整数据库备份，备份失败则不会删除任何数据
- 抽卡数据、导入状态和卡池边界保存在可同步的 `gacha-data.db`；游戏目录与资源缓存保存在设备专属的 `app-state.db`
...
2. 回到首页点击「扫描抽卡」，按数据来源选择六种导入方式之一
3. 对 JSON、截图或批量手动数据完成预检和人工校验，再确认写入
...
- 启用 OneDrive 后，仅将版本化抽卡记录快照保存到用户 OneDrive 根目录下的 `Wuwa Gacha Tool` 文件夹；刷新凭据由系统凭据库保护
- 游戏目录模式仅读取 `Client.log` 以提取官方抽卡链接，不读取游戏账号密码
- 云鸣潮模式需要访问云鸣潮并完成其官方登录流程，工具只提取唤取记录链接
- 截图识别由按需安装的本地 OCR 组件执行，原始截图不会上传；组件和模型可随时删除
- 数据同步使用官方抽卡接口；自动更新会访问 GitHub Release 及配置的下载代理
- OCR 组件、应用更新和资源包会访问各自的 GitHub Release，并沿用应用配置的下载代理
...
- JSON 导入需包含 `uid` 与卡池数据。可参考[本工具 JSON 示例](https://github.com/juliy819/wuwa-gacha-tool/blob/main/docs/examples/uid_999999999_mock_2026-07-31.json)；同时支持 [WaveTools](https://github.com/JamXi233/WaveTools) 的 `info.uid + list[]` 格式，以及兼容官方接口结构的 JSON。WaveTools 的 `gacha_id`、物品编号、星级和抽数即使以字符串保存也会自动转换；未知卡池或缺少关键字段会在预检阶段拒绝，不会静默跳过记录。
- OCR 可能受到截图裁切、遮挡和新角色素材尚未更新等因素影响，导入前必须核对五星、抽数、顺序和卡池
- 截图和批量手动导入会生成模拟记录；日期范围与已有记录重叠时应先确认实际先后关系
...
src-tauri/
├─ src/commands/     Tauri 命令入口与 OCR 组件管理
