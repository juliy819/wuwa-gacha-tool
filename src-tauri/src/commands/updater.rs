...
const UPDATE_PROXIES: &[(&str, fn(&str) -> String)] = &[
    (
        "gh.h233.eu.org",
        |u: &str| format!("https://gh.h233.eu.org/{}", u),
    ),
    (
        "edgeone.gh-proxy.org",
        |u: &str| format!("https://edgeone.gh-proxy.org/{}", u),
    ),
    (
        "cors.isteed.cc",
        |u: &str| format!("https://cors.isteed.cc/{}", u.trim_start_matches("https://").trim_start_matches("http://")),
    ),
    ("GitHub 官方", |u: &str| u.to_string()),
];
...
pub async fn download_and_install_update(
    ...
    // 通知前端下载完成
    let _ = app.emit("update-download-done", serde_json::json!({
...
async fn try_download_once(
    ...
    if file_size < 1024 * 100 {
        ...
        let _ = std::fs::remove_file(&local_path);
        return Err(format!("下载文件过小（{} bytes），可能不是有效安装包", file_size));
    }
...
fn open_installer(path: &std::path::Path) -> std::io::Result<()> {
    if cfg!(target_os = "windows") {
        ...
        Command::new(path).args(["/S", "/R", "/NS"]).spawn().map(|_| ())
    } else if cfg!(target_os = "macos") {
        use std::process::Command;
